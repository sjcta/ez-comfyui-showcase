...
class StatusUiContractTests(unittest.TestCase):
    ...
    def test_service_button_keeps_state_text_compact(self):
        ...
        self.assertIn(".statusbar[data-state=\"busy\"] .svc-btn.running", css)
        self.assertIn(".statusbar[data-state=\"overload\"] .svc-btn.running", css)
    ...
    def test_service_button_preserves_multi_instance_progress_on_job_updates(self):
        ...
        self.assertIn("var _lastStatusInstances = []", status_js)
        self.assertIn("_lastStatusInstances = insts.slice()", status_js)
        self.assertIn("if (statusInstances.length)", status_js)
        self.assertIn("_instanceSummaryHtml(items)", status_js)
