...
class Flux2DevI2IWorkflowTest(unittest.TestCase):
    def test_t2i_turbo_uses_dedicated_turbo_gguf_loader(self):
        workflow = json.loads(T2I_WORKFLOW.read_text())

        self.assertEqual(
            workflow["12"]["inputs"]["unet_name"],
            "gguf/flux2-dev-turbo-Q4_K_M.gguf",
        )
        self.assertNotIn("89", workflow)
        self.assertEqual(
        ...
        self.assertEqual(workflow["26"]["inputs"]["guidance"], 1.2)
        self.assertEqual(workflow["22"]["class_type"], "BasicGuider")
        self.assertEqual(workflow["22"]["inputs"]["model"], ["12", 0])
    ...
    def test_workflow_meta_makes_dev_i2i_visible(self):
        ...
        self.assertEqual(entry["name"], "[实验] Flux2 Dev Turbo Q4 图生图")
