...
class PollManagerResumeTest(unittest.TestCase):
    ...
    def test_recent_local_queued_job_is_not_removed_before_server_poll_catches_up(self):
        script = textwrap.dedent(
            r"""
            const fs = require('fs');
            const vm = require('vm');
            ...
            global.window = {
              ...
              CW: {
                ...
                loadHistory: async () => { calls.loadHistory += 1; },
                loadHistoryNoRender: async () => { calls.loadHistory += 1; },
                forceGalleryRerender: () => { calls.rerender += 1; },
                syncComfyServiceButton: () => {}
              }
            };
            ...
            setImmediate(() => {
              console.log(JSON.stringify({
                jobExists: Object.prototype.hasOwnProperty.call(jobs, 'job-local'),
                loadHistory: calls.loadHistory,
                rerender: calls.rerender
              }));
            });
        ...
        self.assertTrue(data["jobExists"])
        self.assertEqual(data["loadHistory"], 0)
        self.assertEqual(data["rerender"], 0)
    ...
    def test_status_change_keeps_previous_job_until_on_job_update(self):
        ...
        self.assertIn("function _isProtectedLocalSubmit(job)", source)
        self.assertIn("if (_isProtectedLocalSubmit(jobs[cleanId])) continue;", source)
