...
class GalleryJobIsolationContractTests(unittest.TestCase):
    def test_ws_and_render_paths_filter_active_jobs_by_current_user(self):
        ...
        self.assertNotIn("function _isJobVisibleToCurrentUser", card_manager)
    ...
    def test_live_job_patch_reorders_active_cards(self):
        ...
        self.assertIn("function _ensureJobCardOrder", history)
        self.assertIn("_ensureJobCardOrder();", history)
        self.assertIn("if (cards.length < 1) return", history)
        self.assertNotIn("if (cards.length < 2) return", history)
        self.assertIn("_sortJobCards([ja, jb])", history)
        self.assertIn("compareDocumentPosition(firstNonJob)", history)
        self.assertIn("gallery.insertBefore(card, firstNonJob)", history)
        self.assertNotIn("function _ensureJobCardOrder", card_manager)
    ...
    def test_gallery_patch_keeps_unchanged_cards_in_place(self):
        ...
        self.assertIn("function _placeGalleryChild", history)
        self.assertIn("if (child === cursor)", history)
        self.assertIn("gallery.insertBefore(child, cursor)", history)
        self.assertNotIn("gallery.appendChild(oldChild);", history)
        self.assertIn("data-video-mask-bound", history)
        self.assertIn("--gi-info-height", history)
        self.assertNotIn("function _placeGalleryChild", card_manager)

    def test_failed_jobs_remain_until_dismissed_but_cancelled_do_not(self):
        poll_manager = (ROOT / "static/js/modules/poll_manager.js").read_text()
        history = (ROOT / "static/js/modules/history.js").read_text()
        card_manager = (ROOT / "static/js/modules/card_manager.js").read_text()
